...
using Mammothcode.Public.Core.Config;
using Mammothcode.Public.Core.Util;
using Mammothcode.Public.Core.Repository;

namespace Mammothcode.Public.Core.Core
{
	/// <summary>
	...
	public class DefaultRepositorySelector : IRepositorySelector
	{
		...
		/// an appropriate value would be <see cref="Mammothcode.Public.Core.Repository.Hierarchy.Hierarchy"/>.
		/// </para>
		/// </remarks>
		...
		public DefaultRepositorySelector(Type defaultRepositoryType)
		{
			...
			{
				throw Mammothcode.Public.Core.Util.SystemInfo.CreateArgumentOutOfRangeException("defaultRepositoryType", defaultRepositoryType, "Parameter: defaultRepositoryType, Value: [" + defaultRepositoryType + "] out of range. Argument must implement the ILoggerRepository interface");
			}
		...
		/// to create can be overridden by specifying the <see cref="Mammothcode.Public.Core.Config.RepositoryAttribute"/> 
		/// attribute on the <paramref name="repositoryAssembly"/>.
		/// </para>
		/// <para>
		/// The default values are to use the <see cref="Mammothcode.Public.Core.Repository.Hierarchy.Hierarchy"/> 
		...
		/// any <see cref="Mammothcode.Public.Core.Config.ConfiguratorAttribute"/> attributes defined on
		...
		/// <see cref="Mammothcode.Public.Core.Config.RepositoryAttribute"/> attribute on the 
		...
		/// configured using any <see cref="Mammothcode.Public.Core.Config.ConfiguratorAttribute"/> 
		...
		/// Also the <see cref="Mammothcode.Public.Core.Config.RepositoryAttribute"/> attribute on the
		...
		/// </summary>
		/// <param name="assembly">The assembly that has a <see cref="Mammothcode.Public.Core.Config.RepositoryAttribute"/>.</param>
		...
		private void GetInfoForAssembly(Assembly assembly, ref string repositoryName, ref Type repositoryType)
		{
			...
			try
			{
				...
				object[] repositoryAttributes = Attribute.GetCustomAttributes(assembly, typeof(Mammothcode.Public.Core.Config.RepositoryAttribute), false);
				...
				else
				{
					...
					{
						LogLog.Error(declaringType, "Assembly [" + assembly + "] has multiple Mammothcode.Public.Core.Config.RepositoryAttribute assembly attributes. Only using first occurrence.");
					}

					Mammothcode.Public.Core.Config.RepositoryAttribute domAttr = repositoryAttributes[0] as Mammothcode.Public.Core.Config.RepositoryAttribute;
		...
		/// </summary>
		/// <param name="assembly">The assembly containing <see cref="Mammothcode.Public.Core.Config.ConfiguratorAttribute"/>
		...
		private void ConfigureRepository(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Mammothcode.Public.Core.Config.ConfiguratorAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				...
				foreach(Mammothcode.Public.Core.Config.ConfiguratorAttribute configAttr in configAttributes)
				{
					if (configAttr != null)
					{
						try
		...
		private void LoadPlugins(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Mammothcode.Public.Core.Config.PluginAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(Mammothcode.Public.Core.Plugin.IPluginFactory configAttr in configAttributes)
				{
					try
					{
		...
		private void LoadAliases(Assembly assembly, ILoggerRepository repository)
		{
			...
			object[] configAttributes = Attribute.GetCustomAttributes(assembly, typeof(Mammothcode.Public.Core.Config.AliasRepositoryAttribute), false);
			if (configAttributes != null && configAttributes.Length > 0)
			{
				foreach(Mammothcode.Public.Core.Config.AliasRepositoryAttribute configAttr in configAttributes)
				{
					try
					{
