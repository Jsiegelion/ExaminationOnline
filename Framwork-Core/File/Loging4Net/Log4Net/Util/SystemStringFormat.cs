...
namespace Mammothcode.Public.Core.Util
{
	/// <summary>
	...
	public sealed class SystemStringFormat
	{
		...
		private static string StringFormat(IFormatProvider provider, string format, params object[] args)
		{
			...
			{
				Mammothcode.Public.Core.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]", ex);
				return StringFormatError(ex, format, args);
			}
#if NETCF
			catch
			{
				Mammothcode.Public.Core.Util.LogLog.Warn(declaringType, "Exception while rendering format ["+format+"]");
		...
		private static string StringFormatError(Exception formatException, string format, object[] args)
		{
			...
			{
				Mammothcode.Public.Core.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling", ex);
				return "<log4net.Error>Exception during StringFormat. See Internal Log.</log4net.Error>";
			}
#if NETCF
			catch
			{
				Mammothcode.Public.Core.Util.LogLog.Error(declaringType, "INTERNAL ERROR during StringFormat error handling");
				return "<log4net.Error>Exception during StringFormat. See Internal Log.</log4net.Error>";
			}
#endif
		}
