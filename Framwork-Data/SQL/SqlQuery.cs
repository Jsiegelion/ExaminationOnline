...
namespace Mammothcode.Public.Data
{
    public abstract class SqlQuery
    {
        ...
        internal object Param
        {
            get
            {
                if (this._Param != null && this._Param.Count > 0)
                {
                    ...
                    {
                        //2014-10-13 添加类型转换
                        Type conversionType = ((modelType.GetProperty(item.Key).PropertyType));
        ...
        internal SqlQuery AppendParam<T>(T t) where T : class
        {
            ...
            {
                //修复[修改数据时只修改某些字段]的BUG，孙佳杰，2015.1.5
                if (store.Fields.Contains(item.Column))
                {
        ...
        public SqlQuery SetExcProperties<T>(IList<string> properties)
        {
            ...
            this._Param = this._Param.FindAll(n =>
            {
    ...
    public class SqlQuery<T> : SqlQuery where T : class
    {
        ...
        public void SetWhere(List<DapperExSingleSqlQuery<T>> singleSqlQueryList)
        {
            ...
            {
                //sunjayjay 2015.9.16
                if (singleSqlQuery != null)
                {
                    switch (singleSqlQuery.SelectOperation)
                    {
                        case DapperExSingleSqlQuerySelect.And:
                            Where(singleSqlQuery.Expression, singleSqlQuery.Operation, singleSqlQuery.Value, true);
                            break;
                        case DapperExSingleSqlQuerySelect.Or:
                            Where(singleSqlQuery.Expression, singleSqlQuery.Operation, singleSqlQuery.Value, false);
                            break;
                        case DapperExSingleSqlQuerySelect.LeftInclude:
                            LeftInclude(singleSqlQuery.LeftAnd);
                            break;
